...
class DEXExtension(context: ExtensionContext) extends Extension with ScorexLogging {
  ...
  override def start(): Unit = {
    ...
    val host: String = context.settings.config.as[String]("TN.dex.grpc.integration.host")
    val port: Int = context.settings.config.as[Int]("TN.dex.grpc.integration.port")
    val allowedBlockchainStateAccounts: Set[ByteStr] =
      context.settings.config.as[Set[ByteStr]]("TN.dex.order-script-validation.allowed-blockchain-state-accounts")
