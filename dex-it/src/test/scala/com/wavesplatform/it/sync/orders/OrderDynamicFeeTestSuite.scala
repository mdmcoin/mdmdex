...
class OrderDynamicFeeTestSuite extends OrderFeeBaseTestSuite {
    ...
    "have proper balances after matching" in {
      def runTest(feeAsset: Asset, matcherFee: Long): Unit = {
        ...
        eventually {
          ...
          matcherFeeAssetBalance + 2 * matcherFee - (if (feeAsset == Waves) 0.04.waves else 0L)
        }
      }
  ...
  "asset fee is not supported" - {
    ...
    "asset became not supported after order was placed" in {
      ...
      val bobOrder = mkBobOrder(eth, 40000L)
      dex1.api.place(bobOrder)
      dex1.api.deleteAssetRate(eth)
      dex1.api.place(mkAliceOrder(Waves, 0.04.waves))
      dex1.api.waitForOrderStatus(bobOrder, Status.Filled)
      waitForOrderAtNode(bobOrder)

      eventually {
        wavesNode1.api.balance(bob, eth) shouldBe (bobEthBalance - 40000L)
        wavesNode1.api.balance(alice, Waves) shouldBe (aliceWavesBalance - 0.04.waves - 1.waves)
      }
    ...
    "asset became not supported after order was partially filled" in {
      ...
      dex1.api.getReservedBalanceWithApiKey(alice)(eth) shouldBe 40000L
      dex1.api.place(mkBobOrder(eth, 40000L))
      ...
      eventually {
        ...
        wavesNode1.api.balance(alice, eth) shouldBe (aliceEthBalance - 20000L)
        wavesNode1.api.balance(bob, eth) shouldBe (bobEthBalance - 40000L)
      ...
      val bobSecondOrder = mkBobOrder(Waves, 0.04.waves)
      ...
      eventually {
        ...
        wavesNode1.api.balance(alice, eth) shouldBe (aliceEthBalance - 40000L)
        wavesNode1.api.balance(bob, eth) shouldBe (bobEthBalance - 40000L)
        wavesNode1.api.balance(bob, Waves) shouldBe (bobWavesBalance - 0.04.waves + 2.waves)
      }
    }
    ...
    "rates of asset pair was changed while order is placed (reserved balance should stay the same)" in {
      ...
      dex1.api.getReservedBalanceWithApiKey(bob)(eth) shouldBe 40000L
      upsertAssetRate(eth -> ethRate * 2)

      dex1.api.getReservedBalanceWithApiKey(bob)(eth) shouldBe 40000L
      dex1.api.place(mkAliceOrder(eth, 40000L))

      waitForOrderAtNode(bobOrder)

      eventually(wavesNode1.api.balance(bob, eth) shouldBe bobEthBalance - 40000L)
    }
  }
  ...
  "orders with non-waves asset fee" - {
    ...
    "are full filled" in {
      ...
      val aliceOrder = mkAliceOrder(eth, 40000L)
      dex1.api.place(aliceOrder)
      ...
      eventually {
        ...
        wavesNode1.api.balance(alice, eth) shouldBe (aliceEthBalance - 40000L)
        wavesNode1.api.balance(matcher, eth) shouldBe (matcherEthBalance + 2 * 40000L)
      }
    }
    ...
    "are partially filled" in {
      ...
      eventually {
        ...
        wavesNode1.api.balance(alice, Waves) shouldBe (aliceWavesBalance - AcceptedOrder.correctedAmountOfAmountAsset(1.waves, 50000L))
        wavesNode1.api.balance(alice, eth) shouldBe (aliceEthBalance - 20000L)
        wavesNode1.api.balance(matcher, eth) shouldBe (matcherEthBalance + 40000L + 20000L)
      }
  ...
  "cancellation of" - {
    ...
    "order with non-waves fee" in {
      ...
      val order = mkBobOrder(eth, 40000L)
    ...
    "partially filled order with non-waves fee" in {
      ...
      wavesNode1.api.balance(alice, eth) shouldBe (aliceEthBalance - 40000L / 2)
  ...
  "fee in pairs with different decimals count" in {
    ...
    dex1.api.getReservedBalanceWithApiKey(bob) shouldBe Map(usd -> 12L, Waves -> 1.waves)
    ...
    dex1.api.getReservedBalanceWithApiKey(alice) shouldBe Map(usd -> 312)
  ...
  "rounding fee to filled amount" - {
    "if amount cannot be filled" in {
      ...
      withClue("price asset is fee asset") {
        ...
        dex1.safeRestartWithNewSuiteConfig(
          ConfigFactory.parseString(
            s""" TN.dex.order-fee.-1.composite.discount.asset="$WctId" """
          ).withFallback(dexInitialSuiteConfig)
        )
    ...
    "if v2 order filled partially by too low percent of amount" in {
      ...
      val buyOrder = mkOrder(alice, wavesUsdPair, OrderType.BUY, 1000000000.waves, 100, 0.04.waves, version = 2: Byte)

      placeAndAwaitAtDex(buyOrder)

      val sellOrder = mkOrder(bob, wavesUsdPair, OrderType.SELL, 1.waves, 100, 0.04.waves, version = 2: Byte)
    ...
    "if v3 order filled partially by too low percent of amount" in {
      ...
      val buyOrder = mkOrder(alice, wavesUsdPair, OrderType.BUY, 1000000000.waves, 100, 0.04.waves, version = 3: Byte)
    ...
    "percent & fixed fee modes" in {
      ...
      dex1.safeRestartWithNewSuiteConfig(
        ConfigFactory.parseString("TN.dex.order-fee.-1.composite.default.mode = percent").withFallback(dexInitialSuiteConfig)
      )
      check()

      dex1.safeRestartWithNewSuiteConfig(
        ConfigFactory.parseString("TN.dex.order-fee.-1.composite.default.mode = fixed").withFallback(dexInitialSuiteConfig)
      )
      check()
      ...
      dex1.safeRestartWithNewSuiteConfig(
        ConfigFactory
          .parseString(
            s"""
               |TN.dex.order-fee.-1.composite.default {
               |  mode = fixed
               |  fixed.asset = $BtcId
