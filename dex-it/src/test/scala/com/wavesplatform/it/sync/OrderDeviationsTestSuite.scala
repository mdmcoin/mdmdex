...
class OrderDeviationsTestSuite extends MatcherSuiteBase {
  ...
  override protected val dexInitialSuiteConfig: Config = ConfigFactory.parseString(
    s"""
       ...
       |
       |    # TODO COMPAT
       |    profit = $${TN.dex.max-price-deviations.max-price-profit}
       |    loss = $${TN.dex.max-price-deviations.max-price-loss}
       |    fee = $${TN.dex.max-price-deviations.max-fee-deviation}
       |  }
       |  order-fee.-1 {
       |    mode = "percent"
       |    percent {
  ...
  "buy orders price is" - {
    ...
    "in deviation bounds" - {
      for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
        ...
        dex1.api.getReservedBalanceWithApiKey(bob) should matchTo(Map(assetPair.priceAsset -> 710000000L))
    ...
    "out of deviation bounds" - {
      "-- too low" - {
        for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
          ...
          dex1.api.getOrderBook(assetPair).bids shouldBe List(HttpV0LevelAgg(1000.waves, 4000000))
          dex1.api.getReservedBalanceWithApiKey(bob)(assetPair.priceAsset) shouldBe 4008000000L
          ...
          dex1.api.getReservedBalanceWithApiKey(bob) shouldBe Map(assetPair.priceAsset -> 4008000000L)
  ...
  "sell orders price is" - {
    "in deviation bounds" - {
      ...
      for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
        ...
        dex1.api.getReservedBalanceWithApiKey(bob) should matchTo(Map(assetPair.priceAsset -> 304000000L))
