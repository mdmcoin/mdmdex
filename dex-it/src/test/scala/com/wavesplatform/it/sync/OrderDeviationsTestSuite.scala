  ...
  "buy orders price is" - {
    ...
    "in deviation bounds" - {
      for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
        ...
        dex1.api.getReservedBalance(bob) should matchTo(Map(assetPair.priceAsset -> 710000000L))
    ...
    "out of deviation bounds" - {
      "-- too low" - {
        for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
          ...
          dex1.api.getReservedBalance(bob)(assetPair.priceAsset) shouldBe 4008000000L
          ...
          dex1.api.getReservedBalance(bob) shouldBe Map(assetPair.priceAsset -> 4008000000L)
  ...
  "sell orders price is" - {
    "in deviation bounds" - {
      ...
      for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
        ...
        dex1.api.getReservedBalance(bob) should matchTo(Map(assetPair.priceAsset -> 304000000L))
