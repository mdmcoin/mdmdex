...
class TradeBalanceAndRoundingTestSuite extends MatcherSuiteBase {
  ...
  "Alice and Bob trade TN-USD" - {
    ...
    "check waves-usd tradable balance" in {
      ...
      node.tradableBalance(alice, wavesUsdPair)("TN") shouldBe node.accountBalances(alice.address)._1
      ...
      node.tradableBalance(bob, wavesUsdPair)("TN") shouldBe node.accountBalances(bob.address)._1
    }
  }
