    ...
    "positive scenarios of order placement" - {
      "set contracts with AssetPairs/all tx fields/true/one proof and then place order" - {
        for ((sc, i) <- Seq(sc1, sc3, sc4, sc5).zip(Seq(1, 3, 4, 5))) s"$i" in {
          ...
          val aliceOrd1 = mkOrder(alice, predefAssetPair, OrderType.BUY, 500, 2.TN * Order.PriceConstant, smartMatcherFee, version = 2)
      ...
      "place order and then set contract on AssetPairs/true/all fields/one proof" - {
        for ((sc, i) <- Seq(sc1, sc3, sc4, sc5).zip(Seq(1, 3, 4, 5))) s"$i" in {
          val aliceOrd1 =
            mkOrder(alice, predefAssetPair, OrderType.BUY, 500, 2.TN * Order.PriceConstant, matcherFee = smartMatcherFee, version = 2)
          placeAndAwaitAtDex(aliceOrd1)

          val aliceOrd2 =
            mkOrder(alice, aliceWavesPair, OrderType.SELL, 500, 2.TN * Order.PriceConstant, matcherFee = smartMatcherFee, version = 2)
          placeAndAwaitAtDex(aliceOrd2)

          setAliceScript(sc)

          val bobOrd1 = mkOrder(bob, predefAssetPair, OrderType.SELL, 500, 2.TN * Order.PriceConstant, matcherFee = smartMatcherFee, version = 2)
          dex1.api.place(bobOrd1)

          val bobOrd2 = mkOrder(bob, aliceWavesPair, OrderType.BUY, 500, 2.TN * Order.PriceConstant, matcherFee = smartMatcherFee, version = 2)
          ...
          waitForOrderAtNode(bobOrd1).head.getFee shouldBe 4000000
          waitForOrderAtNode(bobOrd2).head.getFee shouldBe 4000000
      ...
      "place order and then set contract with many proofs" in {
        ...
        broadcastAndAwait(mkTransfer(alice, bob, 1000, aliceAsset, 0.06.TN))
        ...
        for ((sc, i) <- Seq(sc5, sc6).zip(Seq(5, 6))) {
          ...
          val bobOrd1 = mkOrder(bob, predefAssetPair, OrderType.SELL, 500, 2.TN * Order.PriceConstant, smartMatcherFee, version = 2)
          dex1.api.place(bobOrd1)

          val bobOrd2 = mkOrder(bob, aliceWavesPair, OrderType.SELL, 500, 2.TN * Order.PriceConstant, smartMatcherFee, version = 2)
          dex1.api.place(bobOrd2)
    ...
    "negative scenarios of order placement" - {
      "set contact and then place order" - {
        for ((sc, i) <- Seq(sc2, sc7, sc8).zip(Seq(2, 7, 8))) s"$i" in {
          ...
          dex1.api
            .tryPlace(mkOrder(alice, predefAssetPair, OrderType.BUY, 500, 2.TN * Order.PriceConstant, smartMatcherFee, version = 2)) should failWith(
            3147522, // AccountScriptDeniedOrder
            MatcherError.Params(address = Some(alice.toAddress.stringRepr))
          )

          dex1.api
            .tryPlace(mkOrder(alice, aliceWavesPair, OrderType.SELL, 500, 2.TN * Order.PriceConstant, smartMatcherFee, version = 2)) should failWith(
            3147522, // AccountScriptDeniedOrder
            MatcherError.Params(address = Some(alice.toAddress.stringRepr))
          )
        }
        ...
        "9" in {
          setAliceScript(sc9)
            dex1.api.tryPlace(mkOrder(alice, predefAssetPair, OrderType.BUY, 500, 2.TN * Order.PriceConstant, smartMatcherFee, version = 2)) should failWith(
      ...
      "place order and then set contract" - {
        for ((contract, i) <- Seq(sc2, sc7, sc8, sc9).zip(Seq(2, 7, 8, 9))) s"$i" in {
          ...
          val bobOrd1 = mkOrder(bob, predefAssetPair, OrderType.SELL, 100, 2.TN * Order.PriceConstant, smartMatcherFee, version = 2)
          dex1.api.place(bobOrd1)

          val bobOrd2 = mkOrder(bob, aliceWavesPair, OrderType.BUY, 500, 2.TN * Order.PriceConstant, smartMatcherFee, version = 2)
          dex1.api.place(bobOrd2)
