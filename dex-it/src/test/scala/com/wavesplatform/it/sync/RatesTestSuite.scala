...
class RatesTestSuite extends MatcherSuiteBase {
  ...
  private def newOrder: Order = mkOrder(alice, wctUsdPair, BUY, amount, price, matcherFee = 4000000, feeAsset = btcAsset)
  ...
  "Rates can be handled via REST" in {
    ...
    dex1.api.tryUpsertRate(Waves, wctRateUpdated) should failWith(20971531, "The rate for TN cannot be changed")
    dex1.api.rates shouldBe defaultRateMap + (wctAsset -> wctRateUpdated)
