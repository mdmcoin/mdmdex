...
class CancelOneOrAllInPairOrdersWithSigSpec extends MatcherSuiteBase with RawHttpChecks {
  ...
  "POST /matcher/orderbook/{amountAsset}/{priceAsset}/cancel" - {
    ...
    "should return an error if publicKey parameter has the different value of used in signature" in {
      ...
      validateIncorrectSignature(dex1.rawApi.cancelOneOrAllInPairOrdersWithSig(bob, "TN", UsdId.toString, o.idStr(), ts, sign))
    }
    ...
    "should return an error if timestamp header has the different value of used in signature" in {
      ...
      validateIncorrectSignature(dex1.rawApi.cancelOneOrAllInPairOrdersWithSig(bob, "TN", UsdId.toString, o.idStr(), ts + 1000, sign))
    }
  }
