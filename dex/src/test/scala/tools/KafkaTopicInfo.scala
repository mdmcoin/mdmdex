...
object KafkaTopicInfo extends App {
  ...
  val config = ConfigFactory
    .parseString("""TN.dex.events-queue.kafka.consumer.client {
    ...
    .withFallback {
      ConfigFactory
        ...
        .resolve()
        .getConfig("TN.dex.events-queue.kafka")
    }

  val consumer = new KafkaConsumer[String, QueueEvent](
    config.getConfig("TN.dex.events-queue.kafka.consumer.client").toProperties,
