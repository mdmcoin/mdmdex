  ...
  "OrderValidator" should {
    ...
    "reject new order" when {
      ...
      "matcherFeeAssetId specified as WAVES Base58 string" in {
        ...
        val json = createOrder(AssetPair(btc, usd), SELL, 100, 3.0, feeAsset = usd).json() ++ Json.obj("matcherFeeAssetId" -> "TN")
      ...
      "matcherFee is not enough (percent mode)" in {
        ...
        withClue("AMOUNT/RECEIVING asset type, min fee = 0.3%, fee should be >= 1.5.waves\n") {
          val order = createOrder(wavesBtcPair, OrderType.BUY, 500.waves, price = 0.0011162, matcherFee = 1.5.waves, feeAsset = Waves)
        ...
        withClue("PRICE/SPENDING asset type, min fee = 0.3%, fee should be >= 0.0016743.btc\n") {
          val order = createOrder(wavesBtcPair, OrderType.BUY, 500.waves, price = 0.0011162, matcherFee = 0.0016743.btc, feeAsset = btc)
          Seq(AssetType.Price, AssetType.Spending).foreach { assetType =>
            validateByPercentSettings(assetType)(order) shouldBe Symbol("right")
            validateByPercentSettings(assetType)(order.updateFee(0.0016742.btc)) should produce("FeeNotEnough")
          }
        }
      }
      ...
      "matcherFee is not enough (fixed mode)" in {
        ...
        withClue("Fee should be >= 0.03.usd\n") {
          val order = createOrder(wavesBtcPair, OrderType.BUY, 100.waves, price = 0.0011162, matcherFee = 0.03.usd, feeAsset = usd)
      ...
      "matcherFee is not enough (dynamic mode)" in {
        val validateByDynamicSettings: Asset => Order => Result[Order] = asset => {
          val cs =
            CompositeSettings(DynamicSettings.symmetric(0.04.waves), discount = Some(CompositeSettings.DiscountAssetSettings(asset, 0.0)))
        ...
        withClue("Fee in USD (2 decimals) should be >= 0.02.usd\n") {
          ...
          validateByDynamicSettings(usd)(order.updateFee(0.14.usd)) should produce("FeeNotEnough")
        }

        withClue("Fee in BTC (8 decimals) should be >= 0.00000034.btc\n") {
          val order = createOrder(wavesBtcPair, OrderType.BUY, 100.waves, price = 0.00011162, matcherFee = 0.0000447.btc, feeAsset = btc)
          validateByDynamicSettings(btc)(order) shouldBe Symbol("right")
          validateByDynamicSettings(btc)(order.updateFee(0.0000010.btc)) should produce("FeeNotEnough")
        }
      }
      ...
      "matcherFee is not enough (blockchain aware)" in {
        ...
        def validateFeeByBlockchain(
          ...
          validateByBlockchain(DynamicSettings.symmetric(0.04.waves))(
        ...
        withClue(s"Fee in USD (2 decimals, rate = 3.7) ") {
          ...
          val order = createOrder(wavesUsdPair, OrderType.BUY, 1.waves, price = 3.7, matcherFee = 0.15.usd, feeAsset = usd)
           ...
           *
           * where
           *   base fee                  = 0.03.waves = 300000,
           *   extra fee for any scripts = 0.04.waves = 400000
           */
      ...
      "order's price is out of deviation bounds (market aware)" in {
        ...
        val buyOrder = createOrder(wavesBtcPair, OrderType.BUY, amount = 250.waves, price = 0.0011081)
        val sellOrder = createOrder(wavesBtcPair, OrderType.SELL, amount = 250.waves, price = 0.0011081)

        val bestAsk = LevelAgg(amount = 800.waves, price = 0.0011082.btc)
        val bestBid = LevelAgg(amount = 600.waves, price = 0.0011080.btc)
         ...
         */
        val lowSellOrderPrices = Array(0, 0.0000001, 0.0000011, 0.00015, 0.0003322, 0.0002999, 0.0003299, 0.0003319, 0.0003323)
        val midSellOrderPrices = Array(0.0003324, 0.0003325, 0.0003999, 0.0006648, 0.0016622, 0.0016623)
        val highSellOrderPrices = Array(0.0016624, 0.001671, 0.0016633, 0.00167, 0.0017, 0.0033248, 0.009, 0.0123123, 0.12312311,
          1.12312312, 123.12312123, 100000.1, 100000.123123, 12345678.12347894)

        val lowBuyOrderPrices = Array(0, 0.0000001, 0.0000033, 0.0000450, 0.000045, 0.0002770, 0.000277, 0.0005539)
        val midBuyOrderPrices =
          Array(0.0005540, 0.0005541, 0.0005580, 0.0005641, 0.0006, 0.001, 0.0017999, 0.0018799, 0.0018829, 0.0018838, 0.0018839)
        val highBuyOrderPrices = Array(0.0018840, 0.0018841, 0.0037678, 0.0123456, 0.01951753, 0.98745612, 1, 1.12345678, 5000.12341234,
        ...
        withClue("buy order price should be >= 0.5 * best bid = 0.5 * 0.0011080.btc = 0.0005540.btc\n") {
          lowBuyOrderPrices.foreach(price => priceValidation(buyOrder.updatePrice(price.btc)) should produce("DeviantOrderPrice"))
        }

        withClue("0.5 * best bid <= buy order price <= 1.7 * best ask (0.0005540.btc <= price <= 0.0018839.btc)\n") {
          midBuyOrderPrices.foreach(price => priceValidation(buyOrder.updatePrice(price.btc)) shouldBe Symbol("right"))
        }

        withClue("buy order price should be <= 1.7 * best ask = 1.7 * 0.0011082.btc = 0.0018839.btc\n") {
        ...
        withClue("sell order price should be >= 0.3 * best bid = 0.3 * 0.0011080.btc = 0.0003324.btc\n") {
          lowSellOrderPrices.foreach(price => priceValidation(sellOrder.updatePrice(price.btc)) should produce("DeviantOrderPrice"))
        }

        withClue("0.3 * best bid <= sell order price <= 1.5 * best ask (0.0003324.btc <= price <= 0.0016623.btc)\n") {
          midSellOrderPrices.foreach(price => priceValidation(sellOrder.updatePrice(price.btc)) shouldBe Symbol("right"))
        }

        withClue("sell order price should be <= 1.5 * best ask = 1.5 * 0.0011082.btc = 0.0016623.btc\n") {
      ...
      "order's fee is out of deviation bounds (market aware)" in {
        ...
        // matherFee = 1% of (amount * price) = 0.00277025 => 0.0027702
        val buyOrder = createOrder(wavesBtcPair, OrderType.BUY, 250.waves, 0.0011081, 0.0027702.btc, feeAsset = btc)
        val sellOrder = createOrder(wavesBtcPair, OrderType.SELL, 250.waves, 0.0011081, 0.0027702.btc, feeAsset = btc)

        val lowBuyOrdersFees = Array(0, 0.0000001, 0.0001, 0.001, 0.0012467, 0.0019999, 0.0023999, 0.0024899, 0.0024929, 0.0024934)
        val validBuyOrdersFees = Array(0.0024935, 0.0024936, 0.002494, 0.0025001, 0.003, 0.0123123, 1.1231231, 123123.1, 123123.12312312)
        ...
        withClue("buy order fee should be >= 0.01 * 0.9 * best ask * amount = 0.01 * 0.9 * 0.0011082.btc * 250 = 0.0024935.btc\n") {
          lowBuyOrdersFees.foreach(fee => feeValidation(buyOrder.updateFee(fee.btc)) should produce("DeviantOrderMatcherFee"))
        }

        withClue("buy order fee >= 0.01 * 0.9 * best ask * amount = 0.01 * 0.9 * 0.0011082.btc * 250 = 0.0024935.btc\n") {
          validBuyOrdersFees.foreach(fee => feeValidation(buyOrder.updateFee(fee.btc)) shouldBe Symbol("right"))
        }

        val lowSellOrdersFees = Array(0, 0.0000001, 0.0001, 0.001, 0.0012467, 0.0019999, 0.0023999, 0.0024899, 0.0024929)
        val validSellOrdersFees = Array(0.0024930, 0.0024931, 0.0024940, 0.0025, 0.003, 0.0123123, 1.1231231, 123123.1, 123123.12312312)
        ...
        withClue("sell order fee should be >= 0.01 * 0.9 * best bid * amount = 0.01 * 0.9 * 0.0011080.btc * 250 = 0.0024930.btc\n") {
          lowSellOrdersFees.foreach(fee => feeValidation(sellOrder.updateFee(fee.btc)) should produce("DeviantOrderMatcherFee"))
        }

        withClue("sell order fee >= 0.01 * 0.9 * best bid * amount = 0.01 * 0.9 * 0.0011080.btc * 250 = 0.0024930.btc\n") {
      ...
      "amount or price does not meet matcher's settings requirements" in {
        ...
        def validateByAmountAndPrice(orderRestrictions: Map[AssetPair, OrderRestrictionsSettings] = orderRestrictions): Order => Result[Order] =
          order => validateByBlockchain(DynamicSettings.symmetric(0.04.waves), orderRestrictions)()(order).value.futureValue
        ...
        withClue(s"Amount restrictions for the Waves/USD pair: step amount = 0.5.waves, min amount = 0.5.waves, max amount = 100.waves\n") {
          Seq(
            ...
            orderWith(amount = 100.0000001.waves, price = 3) // too high amount
          ) foreach { order =>
      ...
      "matcherFee is too small according to two latest rates of fee asset" in {
        val rateCache: RateCache = RateCache(TestRateDb()).futureValue
        val validateByRate: Asset => Order => Result[Order] = asset => {
          val cs = CompositeSettings(DynamicSettings.symmetric(0.03.waves), discount = Some(CompositeSettings.DiscountAssetSettings(asset, 0.0)))
          validateByMatcherSettings(cs, rateCache = rateCache)
        }
      ...
      "matcherFee is enough according to two latest rates of fee asset" in {
        val rateCache: RateCache = RateCache(TestRateDb()).futureValue
        val validateByRate: Asset => Order => Result[Order] = asset => {
          val cs = CompositeSettings(DynamicSettings.symmetric(0.04.waves), discount = Some(CompositeSettings.DiscountAssetSettings(asset, 0.0)))
          validateByMatcherSettings(cs, rateCache = rateCache)
        }
        val order: Order = createOrder(wavesUsdPair, BUY, 1.waves, 3.00, 0.14.usd, feeAsset = usd)
      ...
      "market order price is invalid or tradable balance is not enough for its execution" in {
          ...
          OrderBookAggregatedSnapshot(
            asks = Seq(
              LevelAgg(
                amount = 32.waves,
                price = 0.0011842.btc
              ), // buy part of level, value = 11 * 0.0011842 = 0.0130262.btc, remain to execute =  11 - 11 =  0
              LevelAgg(
                amount = 23.waves,
                price = 0.0011825.btc
              ), // buy whole level,   value = 23 * 0.0011825 = 0.0271975.btc, remain to execute =  34 - 23 = 11
              LevelAgg(
                amount = 36.waves,
                price = 0.0011824.btc
              ), // buy whole level,   value = 36 * 0.0011824 = 0.0425664.btc, remain to execute =  70 - 36 = 34
              LevelAgg(
                amount = 30.waves,
                price = 0.0011810.btc
              ) //  buy whole level,   value = 30 * 0.0011810 = 0.0354300.btc, remain to execute = 100 - 30 = 70
            ).reverse,
            bids = Seq(
              LevelAgg(amount = 85.waves, price = 0.0011808.btc), // close whole level,   value = 85.waves, remain to execute = 100 - 85 = 15
              LevelAgg(amount = 12.waves, price = 0.0011805.btc), // close whole level,   value = 12.waves, remain to execute =  15 - 12 = 3
              LevelAgg(amount = 40.waves, price = 0.0011787.btc), // close part of level, value = 3.waves,  remain to execute =   3 -  3 = 0
              LevelAgg(amount = 15.waves, price = 0.0011782.btc) //  won't be touched
            )
          )

        // cost of 100.TN by the current market state = 0.0354300 + 0.0425664 + 0.0271975 + 0.0130262 = 0.01182201.btc
        ...
        val (buyPrice, sellPrice) = (0.0011850, 0.0011750) // both prices are enough to collapse counter side

        withClue("BUY: in order to buy 100.waves price should be >= 0.0011842.btc, otherwise buy less\n") {
          validateByPrice(createOrder(wavesBtcPair, BUY, amount = 100.waves, price = 0.0011842)) shouldBe Symbol(
            "right"
          ) // the lowest acceptable price for buying 100.waves
          validateByPrice(createOrder(wavesBtcPair, BUY, amount = 100.waves, price = 0.0011841)) should produce(
            "InvalidMarketOrderPrice"
          ) // too low price (can only buy 30 + 36 + 23 = 89.waves)
        }

        withClue("SELL: in order to sell 100.waves price should be <= 0.0011787.btc, otherwise sell less\n") {
          validateByPrice(createOrder(wavesBtcPair, SELL, amount = 100.waves, price = 0.0011787)) shouldBe Symbol(
            "right"
          ) // the highest acceptable price for selling 100.waves
          validateByPrice(createOrder(wavesBtcPair, SELL, amount = 100.waves, price = 0.0011788)) should produce(
            "InvalidMarketOrderPrice"
          ) // too high price (can only sell 85 + 12 = 97.waves)
        }

        withClue("BUY: fee in received asset, required balance: Waves = 0.04, BTC > 0\n") {
          val marketOrder = createOrder(wavesBtcPair, BUY, buyAmount, buyPrice, feeAsset = Waves, matcherFee = 0.03.waves)
          validateByTradableBalance(Map(Waves -> 0.0400000.waves, btc -> 0.0000001.btc))(marketOrder) shouldBe Symbol("right")
          validateByTradableBalance(Map(Waves -> 0.0400000.waves, btc -> 0.0000000.btc))(marketOrder) should produce("BalanceNotEnough")
          validateByTradableBalance(Map(Waves -> 0.0299999.waves, btc -> 0.0000001.btc))(marketOrder) should produce("BalanceNotEnough")
        }

        withClue("BUY: fee in spent asset, required balance: Waves = 0, BTC > 0.0000035\n") {
          val marketOrder = createOrder(wavesBtcPair, BUY, buyAmount, buyPrice, feeAsset = btc, matcherFee = 0.0000035.btc)
          validateByTradableBalance(Map(btc -> 0.0000036.btc))(marketOrder) shouldBe Symbol("right")
          validateByTradableBalance(Map(btc -> 0.0000035.btc))(marketOrder) should produce("BalanceNotEnough")
        }

        withClue("BUY: fee in third asset, required balance: Waves = 0, BTC > 0, ETH = 0.0649308\n") {
          val marketOrder = createOrder(wavesBtcPair, BUY, buyAmount, buyPrice, feeAsset = eth, matcherFee = 0.0649308.eth)
          validateByTradableBalance(Map(btc -> 0.0000001.btc, eth -> 0.0649308.eth))(marketOrder) shouldBe Symbol("right")
          validateByTradableBalance(Map(btc -> 0.0000001.btc, eth -> 0.0649307.eth))(marketOrder) should produce("BalanceNotEnough")
          validateByTradableBalance(Map(btc -> 0.0000000.btc, eth -> 0.0649308.eth))(marketOrder) should produce("BalanceNotEnough")
        }

        withClue("SELL: fee in received asset, required balance: Waves = 0.0000001, BTC = 0.0000035\n") {
          val marketOrder = createOrder(wavesBtcPair, SELL, sellAmount, sellPrice, feeAsset = btc, matcherFee = 0.0000035.btc)
          validateByTradableBalance(Map(Waves -> 0.0000001.waves, btc -> 0.0000035.btc))(marketOrder) shouldBe Symbol("right")
          validateByTradableBalance(Map(Waves -> 0.0000000.waves, btc -> 0.0000035.btc))(marketOrder) should produce("BalanceNotEnough")
          validateByTradableBalance(Map(Waves -> 0.0000001.waves, btc -> 0.0000034.btc))(marketOrder) should produce("BalanceNotEnough")
        }

        withClue("SELL: fee in spent asset, required balance: Waves = 0.0300001, BTC = 0\n") {
          val marketOrder = createOrder(wavesBtcPair, SELL, sellAmount, sellPrice, feeAsset = Waves, matcherFee = 0.03.waves)
          validateByTradableBalance(Map(Waves -> 0.0300001.waves))(marketOrder) shouldBe Symbol("right")
          validateByTradableBalance(Map(Waves -> 0.0300000.waves))(marketOrder) should produce("BalanceNotEnough")
        }

        withClue("SELL: fee in third asset, required balance: Waves = 0.0000001, BTC = 0, ETH = 0.0649308\n") {
          val marketOrder = createOrder(wavesBtcPair, SELL, sellAmount, sellPrice, feeAsset = eth, matcherFee = 0.0649308.eth)
          validateByTradableBalance(Map(Waves -> 0.0000001.waves, eth -> 0.0649308.eth))(marketOrder) shouldBe Symbol("right")
          validateByTradableBalance(Map(Waves -> 0.0000000.waves, eth -> 0.0649308.eth))(marketOrder) should produce("BalanceNotEnough")
          validateByTradableBalance(Map(Waves -> 0.0000001.waves, eth -> 0.0649307.eth))(marketOrder) should produce("BalanceNotEnough")
        }
      }

      "matcherFee is not enough (dynamic settings, different fee for maker and taker)" in {
        val rateCache = RateCache(TestRateDb()).futureValue unsafeTap { _.upsertRate(btc, 0.0011167) }
...
//    }
  ...
  private def validateOrderProofsTest(proofs: Seq[ByteStr]): Unit = {
    ...
    val order =
      OrderV2(
        ...
        amount = 100.waves,
        price = (0.022 * Order.PriceConstant).toLong,
        timestamp = System.currentTimeMillis(),
        expiration = System.currentTimeMillis() + 60 * 60 * 1000L,
        matcherFee = 0.04.waves,
  ...
  private def newBuyOrder: Order = buy(pair = wavesBtcPair, amount = 100.waves, price = 0.022, matcherFee = Some(0.04.waves))
