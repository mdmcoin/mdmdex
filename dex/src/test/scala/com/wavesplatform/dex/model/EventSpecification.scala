...
class EventSpecification extends AnyFreeSpec with Matchers with MatcherSpecBase {
  ...
  "Proper rounding scenario 1" in {
    val counter = sell(wavesBtcPair, 840340L, 0.00000238, matcherFee = Some(4000000L))
    val submitted = buy(wavesBtcPair, 425532L, 0.00000238, matcherFee = Some(4000000L))
  ...
  "Reserved balance should empty after full rounded execution" in {
    val counter = buy(btcEthPair, 923431000L, 0.00031887, matcherFee = Some(4000000), sender = Some(alicePk))
    val submitted = sell(btcEthPair, 223345000L, 0.00031887, matcherFee = Some(4000000), sender = Some(bobPk))
